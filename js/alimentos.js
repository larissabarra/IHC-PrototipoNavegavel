var alimentos = {
  "norte": {
    "frutas": {
      "Abiu": {},
      "acai": {
        "nome": "Açaí",
        "origem": "Amazônia",
        "descricao": "Na região amazônica, o açaí exerce um importante papel socioeconômico e cultural, pois a bebida obtida a partir de seus frutos tem consumo regional elevado e sua exportação tem aumentado muito nestes últimos anos (2). No estado do Maranhão, é conhecido como Jussara.",
        "energia": 262,
        "ptn": 3.6,
        "lip": 2,
        "carb": 57.4,
        "fibra": 32.7,
        "calcio": 118,
        "fosforo": 58,
        "ferro": 1.09,
        "retinol": 11.8,
        "b1": 0.36,
        "b2": 0.01,
        "niacina": 0.4,
        "c": 9
      },
      "Ajuru": {
		"nome": "Ajuru",
        "origem": "Amazônia",
        "descricao": "Fruto arredondado, de cor diversificada entre o branco-creme, o rosa e o púrpura e, algumas vezes quase preto. A polpa é branca,um tanto esponjosa, às vezes adocicada.",
        "energia": 262,
        "ptn": 3.6,
        "lip": 2,
        "carb": 57.4,
        "fibra": 32.7,
        "calcio": 118,
        "fosforo": 58,
        "ferro": 1.09,
        "retinol": 11.8,
        "b1": 0.36,
        "b2": 0.01,
        "niacina": 0.4,
        "c": 9
	  },
      "Araçá": {
        "nome": "Araçá",
        "origem": "Amazônia",
        "descricao": "Fruteira arbustiva que está sendo introduzida na agricultura moderna. Possui fruto arredondado, de coloração verde, amarela ou vermelha, de acordo com a espécie. ",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9},
      "Bacaba": {},
      "Bacuri": {},
      "Banana-pacovã": {},
      "Biribá": {},
      "Buriti": {},
      "Camapu": {},
      "Camu-camu": {},
      "Camutim": {},
      "Castanha do Brasil": {
        "nome": "Castanha do Brasil",
        "origem": "Amazônia",
        "descricao": "O fruto da castanheira, chamada de ouriço, tem peso médio de 750 g e constitui-se em uma resistente cápsula que não se abre espontaneamente, abrigando, em seu interior, um número variado de sementes, entre 10 a 25.",
        "energia": 636,
        "ptn": 14.00,
        "lip": 63.90,
        "carb": 13.00,
        "fibra": 3.40,
        "calcio": 198.00,
        "fosforo": 577.00,
        "ferro": 3.40,
        "retinol": 7.00,
        "b1": 1.09,
        "b2": 0.12,
        "niacina": 1.70,
        "c": 10.0
      },
      "Cubiu": {},
      "Cupuaçu": {},
      "Cupuí": {},
      "Cutite": {},
      "Guaraná": {},
      "Inajá": {},
      "Ingá": {},
      "Jambo": {},
      "Manga": {
	  },
      "Mangaba": {},
      "Maracujá": {},
      "Marajá": {},
      "Murta": {},
      "Piquiá": {},
      "Pupunha": {},
      "Sapota-do-Solimões": {},
      "Sorva": {},
      "Taperebá": {},
      "Tucumã": {},
      "Umari": {},
      "Uxi": {}
    },
    "hortaliças": {
	"Alfavaca":{
		"nome": "Alfavaca",
        "origem": "",
        "descricao": "A alfavaca é uma planta condimentar aromática muito freqüente em vários estados brasileiros. Ocorre também na África, Ásia tropical, Oriente Médio e em todo o Mediterrâneo",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9
	},
	"AZEDINHA":{
		"nome": "AZEDINHA",
        "origem": "",
        "descricao": "Planta de caule vermelho e folhas arreConhecida e utilizada desde a antigüidade, a alfavaca tem grande utilidade culinária. As folhas são usadas como condimento, pois conferem agradá- vel sabor aos molhos, sopas e carnes. No estado do Pará, é muito empregada no preparo do tucupi. O linalol, extraído das folhas, é utilizado como aromatizante pelas indústrias de bebidas, alimentos e perfumaria.",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9
	},
	"BERTALHA":{
		"nome": "BERTALHA",
        "origem": "",
        "descricao": "A procura de opções viáveis para o cultivo nos trópicos, mostrou que a bertalha pode substituir eficientemente o espinafre e o cariru",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9
	},
	"CARURU":{
		"nome": "CARURU",
        "origem": "",
        "descricao": "Utilizam-se as suas folhas e hastes tenras nos mesmos pratos que o espinafre, preparados de forma similar",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9
	},
	"COENTRO":{
		"nome": "COENTRO",
        "origem": "",
        "descricao": "No Brasil, sua importância está associada ao consumo das folhas frescas, utilizadas como condimento, principalmente no Norte e Nordeste.",
        "energia": 62,
        "ptn": 1.5,
        "lip": 0.6,
        "carb": 14.3,
        "fibra": 5.2,
        "calcio": 48,
        "fosforo": 33,
        "ferro":6.3,
        "retinol": 48,
        "b1": 0.06,
        "b2": 0.04,
        "niacina": 1.3,
        "c": 9
	},
    },
    "leguminosas": {

    },
    "tuberculos": {

    }
  },
  "nordeste": {
    "frutas": {

    },
    "hortaliças": {

    },
    "leguminosas": {

    },
    "tuberculos": {

    }
  },
  "sudeste": {
    "frutas": {

    },
    "hortaliças": {

    },
    "leguminosas": {

    },
    "tuberculos": {

    }
  },
  "sul": {
    "frutas": {

    },
    "hortaliças": {

    },
    "leguminosas": {

    },
    "tuberculos": {

    }
  },
  "centrooeste": {
    "frutas": {

    },
    "hortaliças": {

    },
    "leguminosas": {

    },
    "tuberculos": {

    }
  }
};
